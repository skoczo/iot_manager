...
final class JWTTokenService implements Clock, TokenService {
	...
	JWTTokenService(@Value("${jwt.issuer:skoczo}") final String issuer,
			@Value("${jwt.expiration-sec:3600}") final int expirationSec,
			@Value("${jwt.clock-skew-sec:120}") final int clockSkewSec,
			@Value("${jwt.secret:skoczoSuperSecretKey}") final String secret) {
		super();
